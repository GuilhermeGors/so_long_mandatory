...
/*   Updated: 2024/12/13 20:09:32 by gugomes-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_game(t_game *game, int argc, char **argv)
{
	...
	game->mlx = mlx_init();
	if (!game->mlx)
		return (ft_putstr_fd_int("Error! Wrong connection MLX\n.", 2));
	game->win = mlx_new_window(game->mlx, game->map_width * TILE_SIZE, \
		game->map_height * TILE_SIZE, "Game");
    if (!game->win)
    {
	...
	if (!load_images(game))
		return (ft_putstr_fd_int("Error loading textures\n", 2));
	return (1);
}
